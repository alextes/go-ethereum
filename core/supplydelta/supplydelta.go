...
func SupplyDelta(block *types.Block, parent *types.Header, db *trie.Database, config *params.ChainConfig) (*big.Int, error) {
	...
	for fwdIt.Next() {
		...
		supply_delta.Add(supply_delta, acc.Balance)
	}
	...
	for rewIt.Next() {
		...
		supply_delta.Sub(supply_delta, acc.Balance)
	}
	...
	// delta.
	diff := new(big.Int).Set(supply_delta)
	diff.Sub(diff, fixedReward)
	diff.Sub(diff, unclesReward)
	diff.Add(diff, burn)

	log.Info("Calculated supply delta for block", "number", block.Number(), "hash", block.Hash(), "state", supply_delta, "fixedReward", fixedReward, "unclesReward", unclesReward, "burn", burn, "diff", diff, "elapsed", time.Since(start))
	return supply_delta, nil
}
