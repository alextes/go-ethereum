...
func SupplyDelta(block *types.Block, parent *types.Header, db *trie.Database, config *params.ChainConfig) (*big.Int, error) {
	...
	log.Info("Calculated supply delta for block", "number", block.Number(), "hash", block.Hash(), "state", supply_delta, "subsidy", subsidy, "uncles", uncles, "burn", burn, "diff", diff, "elapsed", time.Since(start))
